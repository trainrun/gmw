...
from gfaLib.abstractions import Orientation, GFALine, GFAFormat
...
class GFAParser:
    ...
    def read_gfa_line(datas: list[str], load_sequence_in_memory: bool = True, regexp_pattern: str = ".*", memory_mode: bool = False) -> tuple[str, GFALine, dict]:
        ...
        line_type = GFALine(datas[0])
        if line_type == GFALine.SEGMENT:
            line_datas["length"] = len(datas[2].strip())
            if load_sequence_in_memory:
                line_datas["seq"] = datas[2].strip()
            if not memory_mode:
                supp_data = GFAParser.supplementary_datas(datas, 3)
                if "ST" not in supp_data:
                    supp_data["ST"] = 0
                if "EN" not in supp_data:
                    supp_data["EN"] = 0
                if "TP" not in supp_data:
                    supp_data["TP"] = '-'
                if "OR" not in supp_data:
                    supp_data["OR"] = '?'
                if "AC" not in supp_data:
                    supp_data["AC"] = '-'
                if "DP" not in supp_data:
                    supp_data["DP"] = 1.0
                return (datas[1], line_type, {**line_datas, **supp_data})
            else:
                return (datas[1], line_type, line_datas)
        elif line_type == GFALine.LINK:
            if not memory_mode:
                line_datas["orientation"] = {(Orientation(datas[2]), Orientation(datas[4]))}
                return ((datas[1], datas[3], 1), line_type, {**line_datas, **GFAParser.supplementary_datas(datas, 5)})
            else:
                return (None, None, None)
        elif line_type == GFALine.WALK:
            ...
            try:
                line_datas["path"] = [
                    (node[1:], Orientation(node[0])) for node in datas[6].replace('>', ',+').replace('<', ',-')[1:].split(',')
                ]
            except IndexError:
                line_datas["path"] = []
            try:
                label = search(regexp_pattern, f'{datas[1]}#{datas[2]}#{datas[3]}').group(1)
            except:
                label = datas[3].upper()
            return (label, line_type, {**line_datas, **GFAParser.supplementary_datas(datas, 7)})
        elif line_type == GFALine.PATH:
            parts = datas[1].split('#')
            if len(parts) == 3:
                line_datas["name"], line_datas["origin"], line_datas["id"] = parts
            else:
                line_datas["name"] = datas[1]
            ...
            try:
                line_datas["path"] = [(node[:-1], Orientation(node[-1])) for node in datas[2].split(',')]
            except IndexError:
                line_datas["path"] = []
            try:
                label = search(regexp_pattern, line_datas["id"]).group(1)
            except:
                label = line_datas["id"].upper()
            return (label, line_type, {**line_datas, **GFAParser.supplementary_datas(datas, 7)})
        elif line_type in (GFALine.HEADER, GFALine.ANY):
            ...
            if graph.lines:
                for (source, sink), line in graph.lines.items():
                    ...
                    for alt in line['orientation']:
                        ori1, ori2 = alt
                        gfa_writer.write(
                            f"L\t"+f"{source}\t{ori1.value}\t{sink}\t{ori2.value}{supplementary_text}\n")
            if graph.paths:
